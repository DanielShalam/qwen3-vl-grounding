"""Run inference on multi-class LVIS data (all objects per image in one pass)."""
import json
import re
import argparse
import torch
...
def parse_all_bboxes(response):
    """Extract all bbox_2d with labels from response."""
    boxes = []
    ...
    if boxes:
        return boxes
    # Try reverse order (label before bbox)
...
def run_inference(mode="baseline", config_path="configs/lora_config.yaml", limit=None,
    ...
    predictions = []
    print(f"Running {mode} inference on {len(data)} images...")
    ...
    out_dir = Path(output_dir) if output_dir else Path(f"results/{mode}_multiclass")
